...
static int poll_sensor_data(struct sensors_poll_device_t *sensors_device)
{
	...
	ssize_t count =
		sensors_device->poll(sensors_device, buffer, minBufferSize);
	int i;
	...
	for (i = 0; i < count; ++i) {
		...
		dbg("Acceleration: x= %0.2f,", buffer[i].acceleration.x);
		dbg(" y= %0.2f, ", buffer[i].acceleration.y);
		...
		struct sensors_poll_device_t **mSensorDevice)
{
	...
	for (i = 0 ; i < (size_t)count ; i++)
		(*mSensorDevice)->setDelay(
			*mSensorDevice, list[i].handle, 200 * 1000000);
